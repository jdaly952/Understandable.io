...
const AuthScreen = ({ onLoginSuccess }: { onLoginSuccess: () => void }) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Page navigates away to Google — execution stops here
    } catch (err: any) {
...
function UnderstandableEngine() {
  ...
  useEffect(() => {
    // Handle Google OAuth redirect result (required for iOS WKWebView — popup is blocked)
    getRedirectResult(auth).then((result) => {
      if (result?.user) console.log("Redirect sign-in complete:", result.user.email);
    }).catch(console.error);
  ...
  const understandTopic = async (overrideTopic?: string, isDeeper: boolean = false) => {
    ...
    try {
      ...
      const prompt = isDeeper 
        ? `Generate a 3-6-9 Triangulation (a structured explanation) for the concept: "${baseTopic}". PROVIDE GREATER DEPTH, DETAIL, AND NUANCE than before. Return a JSON object with a "valid" boolean field (true if successful, false if nonsense), and if valid, "hook", "analogy", and "mechanism" fields.`
        : `Generate a 3-6-9 Triangulation for the concept: "${baseTopic}". If the concept is complete nonsense, a non-concept, or unrecognizable, return a JSON with a "valid" field set to false and a "mechanism" field explaining that the topic needs to be clearer. If valid, return "valid": true, "hook", "analogy", and "mechanism" fields.`;
                      ...
                      {showSuccessFeedback ? (
                        ...
                        >
                          <div className="space-y-4">
                            ...
                            <p className="font-sans text-xl opacity-60">We're here to make things understandable.</p>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
                            <button 
                              ...
                              className="p-8 bg-accent text-bg rounded-3xl font-display font-black text-2xl uppercase tracking-tighter hover:scale-105 transition-transform"
                            >
                               I understand now!
                            </button>
                            <button 
                              onClick={() => understandTopic(concept, true)}
                              className="p-8 bg-ink text-bg rounded-3xl font-display font-black text-2xl uppercase tracking-tighter hover:scale-105 transition-transform"
                            >
                               Dig Deeper!
                            </button>
                          </div>
